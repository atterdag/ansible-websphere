...
module: ibmim
short_description:
    - Manage IBM Installation Manager packages
description:
    - Install, Update and Uninstall Installation Manager packages using remote or local repositories
version_added: "1.9.4"
author:
...
options:
  id:
    ...
    repositories:
        description:
            - Comma separated list of repositories to use. May be a path, URL or both.
    properties:
        description:
            - Comma separated list of properties needed for package installation. In the format key1=value,key2=value
    state:
        description:
            - Install a package with 'present'. Uninstall a package with 'absent'. Update all packages with 'update'.
        default: present
        choices:
            - present
            - absent
    ...
    install_fixes:
        description:
            - Install fixes if available in the repositories.
        default: none
        choices:
            - none
            - recommended
...
class InstallationManager():
    ...
    def __init__(self):
        ...
        self.module = AnsibleModule(
            argument_spec=dict(
                ...
                connect_passport_advantage=dict(
                    default=False,
                    type='bool'
                ),
                install_fixes=dict(
    ...
    def getVersion(self, pacakgeId):
        ...
        if child.returncode != 0:
            self.module.fail_json(
                msg="Error getting installed version of package '{0}'".format(
                    pacakgeId),
                stdout=stdout_value,
                stderr=stderr_value,
            )
    ...
    def main(self):
        ...
        connectPassportAdvantage = self.module.params[
            'connect_passport_advantage']
        ...
        if not os.path.exists("{0}/eclipse".format(ibmim)):
            self.module.fail_json(
                msg=
                "{0}/eclipse not found. Make sure IBM Installation Manager is installed and that ibmim is pointing to correct directory.".
                format(ibmim),
            ...
            if not self.isProvisioned(dest, pacakgeId):
                ...
                if not repositories:
                    self.module.fail_json(
                        changed=False,
                        msg=
                        "Param repositories is required when installing packages",
                        module_facts=self.module_facts)

                cmd = ("{0}/eclipse/tools/imcl install {1} "
                       "-repositories {2} "
                       "-acceptLicense "
                       "-stopBlockingProcesses ").format(
                           ibmim, pacakgeId, repositories)

                if dest:
                    cmd = "{0} -installationDirectory {1} ".format(cmd, dest)
                if im_shared:
                    cmd = "{0} -sharedResourcesDirectory {1} ".format(
                        cmd, im_shared)
                if properties:
                ...
                if child.returncode != 0:
                    self.module.fail_json(
                        changed=False,
                        msg="Failed installing package '{0}'".format(
                            pacakgeId),
                        stdout=stdout_value,
                        stderr=stderr_value,
                        module_facts=self.module_facts)
                ...
                self.module.exit_json(
                    changed=True,
                    msg="Package '{0}' installed successfully".format(
                        pacakgeId),
                    stdout=stdout_value,
                    stderr=stderr_value,
                    module_facts=self.module_facts)
        ...
        if state == 'absent':
            ...
            if self.isProvisioned(dest, pacakgeId):
                ...
                cmd = "{0}/eclipse/tools/imcl uninstall {1} ".format(
                    ibmim, pacakgeId)
                ...
                if child.returncode != 0:
                    self.module.fail_json(
                        msg="Failed uninstalling package '{0}'".format(
                            pacakgeId),
                        stdout=stdout_value,
                        stderr=stderr_value,
                        module_facts=self.module_facts)
                ...
                self.module.exit_json(
                    changed=True,
                    msg="Package '{0}' uninstalled successfully".format(
                        pacakgeId),
                    stdout=stdout_value,
                    stderr=stderr_value,
                    module_facts=self.module_facts)
        ...
        if state == 'update':
            ...
            if self.module.check_mode:
                self.module.exit_json(
                    changed=False,
                    msg="All installed packages are to be updated".format(
                        pacakgeId),
                    module_facts=self.module_facts)
